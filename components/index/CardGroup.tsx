...
const CardGroup = ({ type, apiName }: props) => {
  ...
  return (
    <>
      ...
      <ul className='grid grid-cols-1 2xl:grid-cols-7 2xl:gap-x-[50px] xl:grid-cols-5 slg:grid-cols-4 ssm:grid-cols-3 xs:grid-cols-2 gap-x-8 gap-y-[60px] mt-[60px]'>
        {showBooks?.map(({ uuid, title_cn, image, tag, is_adult, point }) => (
          <li
            key={uuid}
            className='justify-self-center flex flex-col justify-center items-center w-[175px] h-[361px]'
          >
            <BookCard title={title_cn} cover={image} tag={tag} uuid={uuid} isAdult={is_adult} point={point} />
          </li>
        ))}
        <li className='justify-self-center w-[175px] h-[246px]' onClick={handleToLink}>
